...
HEARTBEAT_STATUS_QUEUE_MAX_SIZE = 5
...
TARGET = command.Position(0, 0, 10)
...
def main() -> int:
    ...
    heartbeat_status_queue = queue_proxy_wrapper.QueueProxyWrapper(
        mp_manager, HEARTBEAT_STATUS_QUEUE_MAX_SIZE
    )
    ...
    while time.time() - start_time < RUN_DURATION:
        try:
            ...
            main_logger.info(f"Heartbeat status: {status}", True)
        ...
        try:
            cmd = command_output_queue.queue.get_nowait()
            main_logger.info(f"Command: {cmd}", True)
        except queue.Empty:
            pass
